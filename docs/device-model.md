...
- якщо в попередньому snapshot уже є `offline_since`, попередній effective state примусово трактується як `offline` (навіть якщо сирий ARP виглядає як `idle`)
...
- if the previous snapshot already contains `offline_since`, the previous effective state is force-resolved as `offline` (even when raw ARP evidence looks like `idle`)
