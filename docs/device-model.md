...
- `is_random_mac` — deterministic backend-ознака random/local MAC (розрахунок з U/L біта першого октету)
- `mac_vendor` — назва вендора за OUI (або `null`, якщо OUI не знайдено/невалідний MAC); для random/local MAC значення informational-only

Vendor mapping source (offline):

- файл: `app/data/mac_vendors.json`
- завантаження: при першому vendor lookup з кешуванням у процесі
- оновлення: вручну редагуванням/оновленням JSON (key = 6 hex OUI, value = vendor name)
- якщо OUI не знайдено: повертається `null` без runtime запитів у зовнішні API
...
- `is_random_mac` — deterministic backend flag for random/local MAC (derived from the first-octet U/L bit)
- `mac_vendor` — vendor name resolved by OUI (or `null` for invalid/unknown OUI); for random/local MAC the value is informational-only

Vendor mapping source (offline):

- file: `app/data/mac_vendors.json`
- loading: on first vendor lookup with in-process caching
- update flow: manually update JSON (key = 6 hex OUI, value = vendor name)
- unknown OUI behavior: returns `null` with no runtime dependency on external APIs
