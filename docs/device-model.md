...
- `mac_vendor` — назва вендора за OUI (або `null`, якщо MAC random/local або OUI не знайдено)
...
- `mac_vendor` — vendor name resolved by OUI (or `null` when MAC is random/local or OUI is unknown)
